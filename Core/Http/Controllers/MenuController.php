...
class MenuController extends Controller
{
    ...
    try {
      ...
      for ($i = 0; $i < sizeof($data); $i++) {
        ...
        if ($data[$i]->icon == null || $data[$i]->icon == 'null' || $data[$i]->icon == '' ||  $data[$i]->icon == ' ') {
          ...
          if (isset(request()->lang_id) && request()->lang_id != getGeneralSetting('default_language')) {
  ...
  public function updateTreeViewData(Request $request)
  {
    try {
      ...
      if (isset($request['name']) && $request['name'] != "" && isset($request['lang_id']) && $request['lang_id'] == getGeneralSetting('default_language')) {
