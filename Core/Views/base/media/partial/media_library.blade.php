            ...
            <div class="attachment-preview">
                <div class="thumbnail">
                    @if ($media->file_type == 'pdf')
                        <img class="img-thumbnail" src="{{ project_asset('/backend/assets/img/pdf-placeholder.png') }}"
                            alt="{{ $media->alt }}" />
                    @elseif($media->file_type == 'zip')
                        <img class="img-thumbnail" src="{{ project_asset('/backend/assets/img/zip-placeholder.png') }}"
                            alt="{{ $media->alt }}" />
                    @elseif($media->file_type == 'video')
                        <img class="img-thumbnail" src="{{ project_asset('/backend/assets/img/mp4-placeholder.png') }}"
                            alt="{{ $media->alt }}" />
                    @else
                        <img class="img-thumbnail" src="{{ project_asset($media->path) }}" alt="{{ $media->alt }}" />
                    @endif
                </div>
            </div>
