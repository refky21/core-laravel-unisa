            ...
            <div class="card-header justify-content-between">
                ...
                <div class="d-flex">
                    ...
                    <div>
                        ...
                        <ul class="dropdown-menu" style="">
                            <li><a href="{{ route('core.blog.comment') . $all_blog_request }}" class="dropdown-item"> {{ translate('All') }}({{ getCommentCount([$count], $search) }})</a></li>
                            <li><a href="{{ route('core.blog.comment') . '?status=mine' . $if_blog }}" class="dropdown-item">{{ translate('Mine') }}({{ getCommentCount([$count, ['tl_blog_comments.user_id', '=', Auth::user()->id]], $search) }})</a></li>
                            <li><a href="{{ route('core.blog.comment') . '?status=pending' . $if_blog }}" class="dropdown-item">{{ translate('Pending') }}({{ getCommentCount([$count, ['tl_blog_comments.status', '=', config('settings.blog_comment_status.pending')]], $search) }})</a></li>
                            <li><a href="{{ route('core.blog.comment') . '?status=approve' . $if_blog }}" class="dropdown-item">{{ translate('Approve') }}({{ getCommentCount([$count, ['tl_blog_comments.status', '=', config('settings.blog_comment_status.approve')]], $search) }})</a></li>
                            <li><a href="{{ route('core.blog.comment') . '?status=spam' . $if_blog }}" class="dropdown-item">{{ translate('Spam') }}({{ getCommentCount([$count, ['tl_blog_comments.status', '=', config('settings.blog_comment_status.spam')]], $search) }})</a></li>
                            <li><a href="{{ route('core.blog.comment') . '?status=trash' . $if_blog }}" class="dropdown-item">{{ translate('Trash') }}({{ getCommentCount([$count, ['tl_blog_comments.status', '=', config('settings.blog_comment_status.trash')]], $search) }})</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered text-nowrap " id="comment_table">
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                ...
                                @foreach ($comments as $comment)
                                    <tr class="comment_list @if ($comment->status == '2') unapprove @endif">
                                        <td class="vtop">
                                            <div class="form-check">
                                                <input type="checkbox" class="form-check-input comment_id" name="comment_id[]" value="{{ $comment->id }}">
                                            </div>
                                        </td>
                                            ...
                                            <span class="avatar avatar-sm me-2 avatar-rounded">
                                                <?php 
                                                    if (isset($author_image)) {
                                                        $file = getFilePath($author_image);
                                                    }else{
                                                        if ($comment_setting['show_avatars'] == 1) {
                                                            $file = url('/public/comment-author-image/' .$comment_setting['avatar_default'] . '.png');
                                                        }else{
                                                            $file = getFilePath($author_image);
                                                        }
                                                    }
                                                ...
                                                <img src="{{$file}}" alt="" class="comment_img">
                                            </span>
                                            ...
                                            <div class="buttons d-block">
                                                ...
                                                @if ($comment->status == '1' || $comment->status == '2')
                                                    ...
                                                    <a href="{{ route('core.blog.comment.edit', $comment->id) }}"
                                                        class="text-primary">{{ translate('Edit') }}
                                                        |</a>
                                                ...
                                                @else
                                                    ...
                                                    @if ($comment->status == '3')
                                                        <a href="javascript:void(0)"
                                                            class="{{ $comment->previous_status == '1' ? 'text-success' : 'text-warning' }}"
                                        ...
                                        @if (!request()->blog)
                                            <td class="vtop">
                                            <a href="{{ route('core.blog.comment') . '?status=singel_blog_comment&blog=' . $comment->blog_id }}" class="btn btn-info rounded-pill position-relative">
                                            {{ getCommentCount([['tl_blog_comments.blog_id', '=', $comment->blog_id], ['tl_blog_comments.status', '=', config('settings.blog_comment_status.pending')]]) }}
                                                ...
                                                <div class="d-inline-block ms-2">
                                                    <a href="{{ route('core.edit.blog', ['id' => $comment->blog_id, 'lang' => getDefaultLang()]) }}"
                                                        target="_blank"
                                                        class="text-info d-block mb-1">{{ mb_substr($comment->blog->translation('name', getLocale()), 0, 35, 'UTF-8') }}</a>
                                                    <a href="{{url('/blog/'. $comment->blog_permalink )}}" target="_blank"
                                    ...
                                    <ul class="pagination mb-0 flex-wrap">
                                        ...
                                        <li class="page-item">
                                            @if (request()->input('page'))
                                                <a class="page-link" href="{{ $route . '&page=' . request()->input('page') + 1 }}"
                                                    style="{{ request()->input('page') == $comments->lastPage() ? 'pointer-events: none' : '' }}"><i class="ri-arrow-right-s-line align-middle"></i></i></a>
                                            @else
                                                <a class="page-link" href="{{ $route . '&page=2' }}"
                                                    style="{{ 1 == $comments->lastPage() ? 'pointer-events: none' : '' }}">
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button onclick="commentDelete()" id="bulk_button" class="btn btn-danger">{{ translate('Delete') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="bulkAction-modal" class="bulkAction-modal modal fade show" aria-modal="true">
        <div class="modal-dialog modal-sm modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ translate('cancel') }}</button>
                    <button onclick="bulkAction()" id="bulk_button" class="btn btn-danger">{{ translate('Delete') }}</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="javascript:;void(0)" method="post" id="commentReplyModalForm">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ translate('cancel') }}</button>
                        <button onclick="replyCommentSubmit()" id="bulk_button" class="btn btn-danger">{{ translate('Reply') }}</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            var bulk_actions_dropdown =
                    `<br><div id="bulk-action" class="dataTables_length d-flex">
                        ...
                        <select class="form-select form-select-sm bulk-action-selection mr-3">
                            ...
                            option
                        +`</select>
                        <button class="btn btn-secondary btn-sm" onclick="bulkActionConfirmation()">{{ translate('Apply') }}</button>
                    </div>`;
