...
<div class="image-box">
    <div class="d-flex flex-wrap gap-10 mb-3">
        @if (isset($data))
            <div class="preview-image-wrapper">
                <img src="{{ $image_src }}" alt="{{ $placeholder_image_alt }}" width="150" class="preview_image"
                    id="{{ $input }}_preview" />
                <button type="button" title="Remove image" class="remove-btn btn btn-icon btn-outline-danger btn-sm rounded-pill btn-wave waves-effect waves-light 555"
                    id="{{ $input }}_remove"
        ...
        @else
            <div class="preview-image-wrapper">
                <img src="{{ $image_src }}" alt="{{ $placeholder_image_alt }}" width="150" class="preview_image"
                    id="{{ $input }}_preview" />
                <button type="button" title="Remove image" class="remove-btn btn btn-icon btn-outline-danger btn-sm rounded-pill btn-wave waves-effect waves-light d-none"
                    id="{{ $input }}_remove"
                    onclick="removeSelection('#{{ $input }}_preview,#{{ $input }}_id,#{{ $input }}_remove')"><i
                        class="bi bi-x"></i></button>
            </div>
    ...
    <div class="image-box-actions">
        <button type="button" class="btn btn-link btn-wave waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#mediaUploadModal"
            id="{{ $input }}_choose"
