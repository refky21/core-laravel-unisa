@php
    ...
    $number_of_featured_blog = isset($value) && isset($value['number_of_featured_blog']) ? $value['number_of_featured_blog']:'';
@endphp
<form action="#" class=" widget_input_field_form px-3 py-3 bg-white"
    ...
    <div class="form-group @if (!empty($lang) && $lang != getDefaultLang()) area-disabled @endif">
        <label for="number_of_featured_blog">{{ translate('Number of Featured Blog') }}</label>
        <input type="number" class="form-control" id="number_of_featured_blog"
            name="number_of_featured_blog" placeholder="{{ translate('Number of Featured Blog') }}"
            value="{{ $number_of_featured_blog }}" @if (!empty($lang) && $lang != getDefaultLang()) disabled @endif>
    </div>
